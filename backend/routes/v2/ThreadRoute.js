...
router.get("/get-all-thread", async (req, res) => {
  try {
    const queryGet =
      "SELECT th.ThreadId,th.ThreadDescription, th.ThreadDateRelease, thp.TotalLike, thp.TotalComment, thp.TotalShare, us.Username FROM Thread th JOIN ThreadPostHeader thp ON th.ThreadId = thp.ThreadId JOIN Users us ON us.UserId = thp.UserId";
    const threads = await db.query(queryGet);

    // ambil data dari firebase storage
    const threadWithImages = await Promise.all(
...
router.post("/add-new-thread", upload.single("imageName"), async (req, res) => {
  ...
  const res2 = await db.query(sqlquery2, [
    ThreadId,
    ThreadDescription,
    ThreadDateRelease,
  ]);
